...
│   │   ├── swagger.config.ts    # Swagger UI setup serving tsoa-generated OpenAPI spec
│   │   └── swagger.json         # Auto-generated OpenAPI 3.0 specification (generated by tsoa)
│   │
│   ├── middlewares/             # Express HTTP request lifecycle middlewares
│   │   ├── auth.middleware.ts   # Standalone JWT authentication middleware (Bearer header / HttpOnly cookie)
│   │   ├── auth.tsoa.ts         # tsoa expressAuthentication handler for @Security('bearerAuth' | 'cookieAuth')
│   │   ├── error.middleware.ts  # Global error handler returning unified error responses
...
│   │   │   ├── auth.controller.ts    # Decorated controller for Auth endpoints (@Route('auth'))
│   │   │   ├── auth.redis.service.ts # Redis-backed Token Family & Refresh Token Rotation
│   │   │   ├── auth.repository.ts    # Data access for OTP records
│   │   │   ├── auth.route.ts         # Additional routes (Passport Google OAuth redirects)
...
│   │       ├── user.controller.ts    # Decorated controller for User CRUD (@Route('users'))
│   │       ├── user.model.ts         # Mongoose User model with soft delete support
...
│   │
│   ├── routes/                  # Auto-generated Express routing
│   │   └── routes.ts            # Generated routes file produced by tsoa spec-and-routes
│   │
│   ├── utils/                   # Shared helper utilities
│   │   ├── cookie.util.ts       # Secure HttpOnly cookie helper for token management
│   │   ├── format.util.ts       # Message formatting utility with dynamic placeholder arguments
...
│   │
│   ├── ioc.ts                   # tsoa IoC container resolving controller instances from containers
│   └── app.ts                   # Application entry point (Express setup, tsoa RegisterRoutes, error pipeline)
│
...
├── tsconfig.json                # TypeScript compiler configuration (NodeNext, ES2022, experimentalDecorators)
├── tsoa.json                    # tsoa configuration (spec generation, routing, IoC, security, ESM)
├── eslint.config.js             # ESLint configuration and code quality rules
├── .prettierrc                  # Prettier code formatting rules
├── .prettierignore              # Prettier ignore file (ignores dist, node_modules, generated routes/swagger)
